  ...
  try {
    ...
    if (!data || data.length === 0) {
      document.getElementById('menu').innerHTML =
        "<p style='text-align:center;padding:50px'>Menu loading...</p>";
      return;
    }
        ...
        <div class="items-grid">
          ${data
            ...
            <div class="item">
              ${
                item.image_url
                  ? `<img src="${item.image_url}" class="item-img" onerror="this.src='https://via.placeholder.com/300x200/006400/white?text=No+Image'">`
                  : ''
              }
    ...
    document.getElementById('menu').innerHTML =
      "<p style='text-align:center;color:#d32f2f;padding:50px'>No internet connection</p>";
  }
};
...
const updateCart = () => {
  ...
  document
    .querySelectorAll('#floatCount, #cartCount, .cart-badge')
  ...
  if (cart.length === 0) {
    itemsDiv.innerHTML =
      "<p style='text-align:center;color:#888;margin:20px 0'>Your cart is empty</p>";
...
const showCategory = (name) => {
  ...
  document
    ...
    .forEach((s) =>
      s.classList.toggle('active', s.querySelector('h2').textContent === name)
    );
};
...
document.querySelectorAll('.stars i').forEach((s) => {
  s.addEventListener('click', () => {
    ...
    document
      .querySelectorAll('.stars i')
      .forEach(
        (x, i) => (x.className = i < v ? 'fas fa-star filled' : 'far fa-star')
      );
  });
});
